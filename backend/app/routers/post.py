...
@router.get('/all/user/',  response_model=List[schemas.PostResponse]
)
...
@router.get('/todo/user/',  response_model=List[schemas.PostResponse]
)
async def read_todo_posts(db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    posts = db.query(models.Post).join(models.I_will_do_it).filter(models.I_will_do_it.user_id == current_user.id).options(joinedload(models.Post.owner)).order_by(models.Post.id.desc()).all()
    ...
    for post in posts:
        ...
        i_will_do_it_count = db.query(models.I_will_do_it).filter(models.I_will_do_it.post_id == post.id).count()
        is_i_will_do_it = True # Since we are only fetching posts that the user marked as "I_will_do_it"
...
@router.get('/done/user/',  response_model=List[schemas.PostResponse]
)
async def read_done_posts(db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    posts = db.query(models.Post).join(models.I_did_it).filter(models.I_did_it.user_id == current_user.id).options(joinedload(models.Post.owner)).order_by(models.Post.id.desc()).all()
    ...
    for post in posts:
        ...
        i_did_it_count = db.query(models.I_did_it).filter(models.I_did_it.post_id == post.id).count()
        is_i_did_it = True # Since we are only fetching posts that the user marked as " I did it"
